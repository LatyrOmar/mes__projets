...
<body style="font-family: 'Poppins' ">
    ...
    <div class="container">
        ...
        <div class="d-flex justify-content-center ">
            <div class="col-7">  
                @error('name')  <div class="alert alert-danger alert-dismissible fade show" role="alert"> 
                ...
                <div class="formulaire">
                    <form action="{{route('Product.store')}}" method="post" enctype="multipart/form-data"> 
                        @csrf             
                        <input class="form-control mb-2" type="text" placeholder="Nom" name="name"  value="">
                        @error('name') <p class="text text-danger">{{$message}}</p>@enderror                     
                        <textarea class="form-control" placeholder="Description" name="description" style="height: 100px"></textarea></br>
                        @error('description') <p class="text text-danger">{{$message}}</p>@enderror
                        <input class="form-control mb-2" type="number" placeholder="Prix €" name="price" style="width: 150px" value="">
                        @error('price') <p class="text text-danger">{{$message}}</p>@enderror
                        <input class="form-control mb-3" type="number" placeholder="Quantité" name="quantity" style="width: 150px" value="" min=1>
                        ...
                        <br><label for="formFile" class="form-label">inserer l'image du produit</label><br>
                        <input class="form-control" type="file" id="formFile" accept="image/*" name="file" multiple >
                        @error('file') <p class="text text-danger">{{$message}}</p>@enderror
                        <button type="submit" class="btn btn-success mt-2 mb-3" name="submit">Ajouter</button>
